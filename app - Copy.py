...
# Stores normal private messages so chats survive refreshes,
# browser closing/reopening, and offline periods.
def initialize_chat_messages_table():
    ...
    # Keeps track of the last message each user has read from each friend.
    # This makes unread counts persistent across refreshes and offline periods.
    cur.execute("""
...
with app.app_context():
    initialize_chat_messages_table()
...
def get_messages():
    ...
    for sender, receiver_name, message, created_at in rows:
        messages.append({
            "sender": sender,
